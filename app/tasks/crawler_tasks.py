...
def crawl_url(
    ...
    try:
        ...
        if hasattr(self, "request") and getattr(self.request, "id", None):
            self.update_state(
                state="PROGRESS", meta={"status": "Starting crawl", "progress": 0}
            )
        ...
        logger.info(
            f"✅ SPA Detection: Website {url} detected as {'SPA' if is_spa else 'static HTML'}"
        )
        ...
        logger.info(
            f"✅ Crawl completed. Content stored via API for website_id: {website_id}"
        )
        ...
        if hasattr(self, "request") and getattr(self.request, "id", None):
            self.update_state(
                state="PROGRESS", meta={"status": "Content stored", "progress": 100}
            )
        ...
        if hasattr(self, "request") and getattr(self.request, "id", None):
            self.update_state(
                state="FAILURE", meta={"error": str(exc), "website_id": website_id}
            )
...
def process_crawled_content(self, website_id: str, pages_count: int) -> Dict[str, Any]:
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"status": "Requesting embeddings processing", "progress": 0},
        )

        logger.info(
            f"Calling backend API to process embeddings for website_id: {website_id}"
        )
        ...
        self.update_state(
            state="PROGRESS", meta={"status": "Embeddings processed", "progress": 100}
        )
...
def schedule_crawl(self, website_id: str, crawl_frequency: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Scheduling crawl for website {website_id} with frequency {crawl_frequency}"
        )
...
@celery_app.task(
    bind=True, base=CrawlerTask, name="crawler.tasks.update_knowledge_base"
)
def update_knowledge_base(
    self, website_id: str, updated_pages: List[str]
) -> Dict[str, Any]:
    """
...
@celery_app.task(
    bind=True, base=CrawlerTask, name="crawler.tasks.monitor_schedule_changes"
)
...
@celery_app.task(
    bind=True, base=CrawlerTask, name="crawler.tasks.cleanup_old_crawl_data"
)
def cleanup_old_crawl_data(self, days_to_keep: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        pages_result = (
            ...
            .delete()
            .lt("scraped_at", cutoff_date.isoformat())
            .execute()
        )
...
@celery_app.task(
    bind=True, base=CrawlerTask, name="crawler.tasks.health_check_websites"
)
def health_check_websites(self) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Health check complete: {healthy_count}/{len(health_results)} websites healthy"
        )
        ...
        raise self.retry(
            exc=exc, countdown=1800, max_retries=2
        )  # Retry after 30 minutes


@celery_app.task(bind=True, base=CrawlerTask, name="crawler.tasks.capture_screenshot")
def capture_screenshot(self, website_id: str, url: str) -> Dict[str, Any]:
    """
    Capture screenshot of a website and upload to backend.
    ...
    try:
        ...
        logger.info(f"Starting screenshot capture for website {website_id}: {url}")
            ...
            try:
                async with async_playwright() as p:
                    ...
                    context = await browser.new_context(
                        ...
                        user_agent="ChatLite-Screenshot-Bot/1.0",
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Page load timeout or error for {url}: {e}. Proceeding with screenshot anyway."
                        )
        ...
        # Run async capture
        screenshot_bytes = asyncio.run(capture())

        # Convert to base64
        screenshot_base64 = f"data:image/png;base64,{base64.b64encode(screenshot_bytes).decode('utf-8')}"

        logger.info(
            f"Screenshot captured successfully for {website_id}, uploading to backend..."
        )

        # Upload to backend
        success = api_client.upload_screenshot(website_id, screenshot_base64)

        if success:
            logger.info(f"Screenshot uploaded successfully for website {website_id}")
            return {
                "status": "completed",
                "website_id": website_id,
                "url": url,
            ...
            logger.error(f"Failed to upload screenshot for website {website_id}")
            return {
                "status": "failed",
                "website_id": website_id,
                "error": "Failed to upload screenshot to backend",
            }

    except Exception as exc:
        logger.error(f"Screenshot capture failed for website {website_id}: {exc}")
        # Don't retry aggressively - screenshots are nice-to-have
        if self.request.retries < 1:
            raise self.retry(
                exc=exc, countdown=300, max_retries=1
            )  # Retry once after 5 minutes
        else:
            return {"status": "failed", "website_id": website_id, "error": str(exc)}
